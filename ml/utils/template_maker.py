...
def create_template(name, age, phone, blood_group, diagnosis, confidence_level, feedback, medicine):
    ...
    draw.text(date_coords, str(datetime.date.today()), (0, 0, 0))
    draw.text(name_coords, name, (0, 0, 0))
    draw.text(age_coords, str(age), (0, 0, 0))
    draw.text(phone_coords, phone, (0, 0, 0))
    draw.text(blood_group_coords, blood_group, (0, 0, 0))
    draw.text(diagnosis_coords, diagnosis, (0, 0, 0))
    draw.text(confidence_level_coords, confidence_level + "%", (0, 0, 0))

    y = 1034
    for feed in feedback:
        draw.text((134, y), feed, (0, 0, 0))
        y += 50

    # Map through the medicine which will be a list of medicines and print them on the prescription
    y = 1619
    for med in medicine:
        draw.text((155, y), med, (0, 0, 0))
        y += 50
